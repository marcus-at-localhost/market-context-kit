#!/usr/bin/env python3
"""
Marketing Report PDF Generator — Market Context Kit
...
def apply_pdf_metadata(canvas, metadata):
    """Set custom PDF properties only when project metadata is enabled."""
    if not isinstance(metadata, Mapping):
        return
    creator = "; ".join((
        str(metadata["toolkit"]),
        str(metadata["host"]),
        f'{metadata["llm_provider"]} {metadata["llm_model"]}',
    ))
    canvas.setAuthor(str(metadata["document_author"]))
    canvas.setCreator(creator)
    canvas.setProducer(creator)
    canvas.setTitle("Marketing Audit Report")
    canvas.setSubject(f'Report prepared with {metadata["toolkit"]}')
...
def generate_report(data, output_path):
    ...
    if isinstance(report_metadata, Mapping):
        elements.append(Paragraph(f'Prepared by: {report_metadata["prepared_by"]}', subtitle_style))
        elements.append(Paragraph(
            f'Generated with: {report_metadata["host"]}; '
            f'{report_metadata["llm_provider"]} {report_metadata["llm_model"]}',
            subtitle_style,
        ))
    ...
    if isinstance(report_metadata, Mapping):
        elements.append(Spacer(1, 0.5 * inch))
        elements.append(Paragraph(
            f'{report_metadata["toolkit"]} · {report_metadata["host"]} · '
            f'{report_metadata["llm_provider"]} {report_metadata["llm_model"]}',
            ParagraphStyle("Footer", parent=body_style, fontSize=8, textColor=COLORS["text_light"])
        ))

    # Build PDF
    doc.build(elements, onFirstPage=lambda canvas, _doc: apply_pdf_metadata(canvas, report_metadata))
    return output_path
