...
Assemble the results into a table, one row per URL analyzed. `Schema types` comes from `analysis.technical.schema_types` in the JSON — top-level nodes only. Entities one level down live in `analysis.technical.schema_types_nested`, a `{type: count}` map covering an `Article`'s `author` Person, an `Organization`'s `subOrganization` LocalBusiness entries, a `FAQPage`'s questions. Microdata `itemtype` and RDFa `typeof` land in `schema_types_microdata`, and blocks that failed to parse in `schema_parse_errors`. Read all four before calling a schema type absent, and never report "no schema" while `schema_parse_errors` is non-empty — that page carries invalid markup, which is a different finding. Nested counts include `@id` references, so a type may be counted more often than it is defined.
