...
- **the Structural Facts block from 1.1**, in every one of the five prompts, with this instruction attached: "Any H1, heading hierarchy, meta tag, canonical, or schema claim in your output must come from this table. Do not derive it from your own WebFetch read — WebFetch reliably misreads SVG titles, aria-labels and hidden text as page structure. If a page you need isn't in this table, say so as a gap rather than asserting structure for it." This is the fix for the 2026-08-15 idt-dichtungen.de run, where three of five subagents reported a WebFetch-sourced SVG logo title as an identical H1 on 459 pages.
- **the Data Manifest from 0.1**, verbatim, in every one of the five prompts — including the `Empty.` form. This is the fix for the second half of the 2026-08-15 idt-dichtungen.de run, where `market-conversion` located an unrelated analytics file and a previous audit report on its own and calibrated its score against them.
